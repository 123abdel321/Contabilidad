							...
							<table>
								<tr>
									<td class="consecutivo padding5">
										<p> {{ $documento->recibo->comprobante->nombre }} <br>
											<span span class="numero-consecutivo">N° {{ $documento->recibo->consecutivo }}</span>
											@if ($documento->nit && $documento->nit->apartamentos)
											<br/>
											<span span class="ubicacion-factura">{{ $documento->nit->apartamentos }}</span>
											@endif
										</p>
									</td>
									<td class="empresa padding5">
										...
										@endif
										@if ($documento->recibo->fecha_manual)
											<span>FECHA: {{ $documento->recibo->fecha_manual }}</span><br>
										@endif
									</td>
								...
								<tr>
									<td class="aling-top padding5">
										<table>
											...
											<tbody>
												<tr>
													<td class="padding3">{{ $documento->nit->nombre_nit }}</td>
												</tr>
												<tr>
													<td class="padding3">{{ $documento->nit->tipo_documento }} N° {{ $documento->nit->numero_documento }}</td>
												</tr>
												<tr>
													<td class="padding3">{{ $documento->nit->direccion }}
														@if($documento->nit->ciudad)
															{{ $documento->nit->ciudad }}
														@endif
													</td>
												</tr>
												@if ($documento->nit->telefono)
													<tr>
														<td class="padding3"> TEL: {{ $documento->nit->telefono }}</td>
													</tr>
												@endif
											</tbody>
										</table>
										...
										<table>
											...
											<tbody>
												<tr >
													...
													<td class="valor padding5">{{ number_format($documento->saldoAnterior, 2) }}</td>
												</tr>
												<tr >
													<td class="padding5">Total abono</td>
													<td class="valor padding5">{{ number_format($documento->recibo->total_abono, 2) }}</td>
												</tr>
												<tr >
													<td class="padding5">Saldo pendiente</td>
													<td class="valor padding5">{{ number_format($documento->saldo, 2) }}</td>
												</tr>
											</tbody>
										</table>
									</td>
										...
										<table class="width-100">
											...
											<tbody>
												...
												<tr>
													...
													<td class="font-13" style="width:45%;text-align:right;">{{ number_format($documento->pagos->sum('valor')) }}</td>
												</tr>
												<tr>
