...
class ExtractoExport implements FromView, WithColumnWidths, WithStyles, WithColumnFormatting, ShouldQueue
{
    use Exportable;

    protected $empresa;
    protected $id_extracto;
	...
	{
        $cabeza = InfExtracto::find($this->id_extracto);
        $documentos = InfExtractoDetalle::where('id_extracto', $this->id_extracto)->get();
        $nit = $cabeza->id_nit ? Nits::find($cabeza->id_nit) : null;

		return view('excel.extracto.extracto', [
			'documentos' => $documentos,
            'encabezado' => (object)[
                'nombre_informe' => 'Extracto',
