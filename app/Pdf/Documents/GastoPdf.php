...
use App\Pdf\Mappers\GastoPdfMapper;
use App\Models\Empresas\Empresa;
use App\Models\Sistema\ConGastos;

class GastoPdf
{
    public static function build(ConGastos $gasto, Empresa $empresa, string $claveUrl): Document
    {
        $data = GastoPdfMapper::map($gasto, $empresa, $claveUrl);

        $document = new Document();
        $document->setConfig([
            'orientation' => $gasto->comprobante->tipo_impresion == 1 ? 'landscape' : 'portrait',
            'paper' => 'A4',
            'filename' => 'gasto_' . uniqid() . '.pdf',
            'empresa' => $empresa,
        ]);
